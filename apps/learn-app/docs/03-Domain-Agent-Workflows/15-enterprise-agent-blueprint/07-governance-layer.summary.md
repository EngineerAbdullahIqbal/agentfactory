### Core Concept
Governance in Cowork has four specific mechanisms: IAM-integrated permissions (who can access the plugin and at what level), immutable audit trails (every interaction logged and unmodifiable), shadow mode (30-day minimum, 95% accuracy threshold before autonomous operation), and human-in-the-loop gates (structural stops for actions requiring human accountability). Governance is not what limits deployment — it is what makes deployment possible in regulated industries.

### Key Mental Models
- **IAM-Inherited Permissions**: Cowork inherits access control from the organisation's IAM system (Azure AD, Okta). Users not in the IAM system cannot see the plugin. Role-based scoping further restricts capabilities within the authorised population.
- **Audit Trails as Defensibility Assets**: Every query, data source accessed, output generated, timestamp, and user identity is logged immutably. In regulated industries, this turns a potential compliance incident into a documented, defensible process.
